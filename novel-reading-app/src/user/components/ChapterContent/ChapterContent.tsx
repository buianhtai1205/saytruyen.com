...
const ChapterContent: React.FC<ChapterContentProps> = ({
    ...
    // Callback to handle closing the modal from ListChapter
    ...
    return (
        <div className={clsx(styles.chapterContentContainer)}>
            <header className={clsx(styles.chapterHeader)}>
                <h1 className={clsx(styles.novelTitle)}>{storyInfo.name}</h1>
                <div className={clsx(styles.author)}>{storyInfo.authorId}</div>
            ...
            <main className={clsx(styles.chapterBody)}>
                ...
                <p className={clsx(styles.paragraph)}>---</p>
                <p className={clsx(styles.paragraph)}>
                    Vô Lượng Đại Hư Kiếm Tổ nhìn xem trên bàn ấm trà, ngữ khí
                    chậm rãi nói, mà Cố An nghiêm túc nghe.
                </p>
                <p className={clsx(styles.paragraph)}>
                    Xuyên việt tọa ủng mười tỉ tài sản, bắt đầu một cái gọi cảm
                    quyến rũ thê tử, bảy cái phong hoa tuyệt đại nữ nhi. Nhưng
                    mà Hứa Hạo lại hoảng sợ được một nhóm. Bởi vì thân phận của
                    hắn là đại phản phái, kết cục là nghịch tử nhân vật chính
                    trở về, giết hắn chiếm lấy thê nữ. Cũng may còn có thời gian
                    hai năm rưỡi, Hứa Hạo có thể trước giờ bỏ cục.
                </p>
                <p className={clsx(styles.paragraph)}>
                    Mời đọc
                    <span className={clsx(styles.paragraph, styles.referStory)}>
                        Trước Giờ Xuyên Thành Phản Phái, Bức Nhân Vật Chính Mụ
                        Mụ Sinh Hài Tử
                    </span>
                </p>
            </main>
